...
function(x,leg.pos,Biom.tstep,Biom.xpos,Biom.ypos,...)
    {
     ...
     Biom       <- if(Biom.tstep == "fin")
                     {
                      round(mean(tail(x$Model$Results[,dim(x$Model$Results)[2]],5)))
                     }
                   else if(Biom.tstep=="mid")
                     {
                      round(mean(x$Model$Results[,dim(x$Model$Results)[2]][c(floor(median(index-2)),floor(median(index-1)),floor(median(index)),floor(median(index+1)),floor(median(index+2)))]))
                     }
                   else
                     {
                      round(mean(head(x$Model$Results[,dim(x$Model$Results)[2]],5)))
                     }
     options(warn=-1)
     for(i in 1:length(fleet.name))
       ...
       obscat <- x$Model$Results[,(i*3+i-1)]
       modcat <- x$Model$Results[,(i*4)]
       resids <- x$Model$Results[,(i*4+1)]
       ...
       text(x=Biom.xpos*max(period), y=Biom.ypos*max(obscat), adj=0,lab=list(bquote("Biom.End_(tons)" ==.(Biom))))
       if(x$Model$Type[i] != 0)
         {
          if(i == 1)
            {
