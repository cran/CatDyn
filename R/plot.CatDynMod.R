...
function(x,leg.pos,Biom.tstep,Biom.xpos,Biom.ypos,AIC.xpos,AIC.ypos,...)
    {
     ...
     for(i in 1:length(fleet.name))
       ...
       mtext(side=3,outer=TRUE,text=paste("Fleet = ",fleet.name[i],", Perturbations = ",x$Model$Type[i],", Distribution = ",gsub("(\\w)(\\w*)", "\\U\\1\\L\\2", x$Model$Distribution, perl=TRUE),", Numerical algorithm = ", x$Model$Method, sep=""))
       options(warn=0)
