...
function(x,step,fleet.name,coleff,colcat,colmbw,unitseff,unitscat,unitsmbw,nmult,season.dates)
   {
    ...
    if(length(unitscat) > 1 || length(unitsmbw) > 1) {stop("The catches and mean body weight of all fleets must be in the same units")}
    ...
    if(step == "day")
    ...
    else if(step == "week")
      {
       ...
       for(i in 1:length(fleet.name))
         {
         if(length(start:(start+end)) != length(x[,coleff[i]]))
